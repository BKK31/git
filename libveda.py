...
def tag_create(repo, name, ref, create_tag_object=False):
    ...
    if create_tag_object:
        ...
        tag.kvlm[b'tagger'] = b'VEDA <veda@example.com>'
        tag.kvlm[None] = b"A tag generated by veda, which won't let you customize the message!"
...
argsp.add_argument("--veda-type", metavar="type", dest="type", choices=["blob", "commit", "tag", "tree"], default=None, help="Specify the expected type")
...
def index_read(repo):
    ...
    assert version == 2, "veda only supports index file version 2"
...
argsp.add_argument("-m", metavar="message", dest="message", help="Message associate with this commit")
...
def tree_from_index(repo, index):
    ...
    for path in sorted_paths:
        ...
        # a pair (basename, SHA)
        parent = os.path.dirname(path)
        base = os.path.basename(path) # The name without the path
...
def cmd_commit(args):
    ...
    if active_branch: # If we're on a branch, we update refs/heads/BRANCH
        with open(GitRepository.repo_file(repo,  os.path.join("refs/heads", active_branch)), "w") as fd:
            fd.write(commit + "\n")
